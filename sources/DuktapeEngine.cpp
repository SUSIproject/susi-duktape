...
void Susi::Duktape::JSEngine::start() {
  ...
  _kickoffKeepAlive();
  _runloop = std::move(std::thread{[this]() { _ioservice.run(); }});
  ...
  if (_script != "") {
    ...
    if (duk_peval_file(ctx, _script.c_str()) != 0) {
      std::cerr << "processing usersource: " << duk_safe_to_string(ctx, -1)
                << std::endl;
    }
  }

  std::cout << "started Susi::duktape engine and loaded " << _script
            << std::endl;
}
...
void Susi::Duktape::JSEngine::init() {
  ...
  if (duk_peval_lstring(ctx, Susi::Duktape::susiJS.c_str(),
                        Susi::Duktape::susiJS.size()) != 0) {
    std::cerr << "processing susiJS: " << duk_safe_to_string(ctx, -1)
              << std::endl;
  }
}
...
duk_ret_t Susi::Duktape::JSEngine::js_setTimeout(duk_context *ctx) {
  ...
  auto timeout = std::make_shared<boost::asio::deadline_timer>(
      enginePtr->_ioservice, boost::posix_time::milliseconds(arg));
...
void Susi::Duktape::JSEngine::registerProcessor(std::string topic) {
  registerIdsProcessor[topic] =
      _client.registerProcessor(topic, [this, topic](Susi::EventPtr event) {
...
void Susi::Duktape::JSEngine::registerConsumer(std::string topic) {
  Susi::Consumer consumer = [this, topic](Susi::SharedEventPtr event) {
    ...
    if (duk_pcall(ctx, 1 /*nargs*/) != 0) {
      std::cerr << (std::string{"Error: "} + duk_safe_to_string(ctx, -1))
                << std::endl;
    }
    duk_pop(ctx); /* pop result/error */
  };
